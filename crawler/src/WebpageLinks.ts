...
export function removeLinksHashes(links: WebpageLink[]): WebpageLink[] {
...
export function filterQueryParamLinks(links: WebpageLink[]): WebpageLink[] {
    return links.filter(link => new URL(link.url).search === "");
}

export function normalizeLinks(url: string, links: WebpageLink[]): WebpageLink[] {
    return links.map(link => normalizeLink(url, link));
}

export function normalizeLink(url: string, link: WebpageLink): WebpageLink {
    return {
        ...link,
